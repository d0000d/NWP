...
if (isset($_POST['_action_']) && $_POST['_action_'] == 'add_news') {
    ...
    $result = mysqli_query($mysqli, $query); // Changed $MySQL to $mysqli

    $ID = mysqli_insert_id($mysqli); // Changed $MySQL to $mysqli
    ...
    if ($_FILES['picture']['error'] == UPLOAD_ERR_OK && $_FILES['picture']['name'] != "") {
        ...
        if ($ext == '.jpg' || $ext == '.png' || $ext == '.gif') { # test if format is picture
            ...
            $_result = mysqli_query($mysqli, $_query); // Changed $MySQL to $mysqli
...
if (isset($_GET['delete']) && $_GET['delete'] != '') {
    ...
    unlink("news/".$row['picture']); // Removed @ before unlink
...
if (isset($_GET['id']) && $_GET['id'] != '') {
    ...
    $result = mysqli_query($mysqli, $query); // Changed $MySQL to $mysqli
    $row = mysqli_fetch_array($result); // Changed @mysqli_fetch_array to mysqli_fetch_array
    print '
    <h2>News overview</h2>
    <div class="news">
...
# Edit news
    ...
    <div id="Vijesti">
        <table>
            ...
            $query .= " ORDER BY date DESC";
            $result = mysqli_query($mysqli, $query); // Changed $MySQL to $mysqli
            while($row = mysqli_fetch_array($result)) { // Changed @mysqli_fetch_array to mysqli_fetch_array
                print '
                <tr>
...
mysqli_close($mysqli); // Changed @mysqli_close to mysqli_close
?>
